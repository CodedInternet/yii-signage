...
{
    public $types = array('ticker', 'media', 'text');
        ...
        return array(
            ...
            array('allow',
                ...
                'actions' => array('view', 'add', 'browse', 'upload',
                    'postfile', 'delete', 'update', 'moderate', 'info'),
                'users' => array('@'),
            ),
    ...
    public function actionIndex()
    {
        if (Yii::app()->user->isGuest) {
            ...
            $currentCount = $model->count('start<=CURDATE() AND end>=CURDATE()');
            $approvedCount = $model->count('start>CURDATE() AND end>=CURDATE()');
    ...
    public function actionAdd()
    {
        ...
        if (isset($_POST['Content'])) {
            ...
            if (empty($_POST['Content']['files'])) {
                ...
                if ($model->content_type == 'media') {
                    $model->media = CUploadedFile::getInstance($model, 'media');
                    $t = time();
                    $model->content = "{$model->user_id}/{$t}-{$model->media->name}";
                }
                ...
                if ($model->save()) {
                    if ($model->content_type == 'media') {
                        $model->media->saveAs(Yii::app()->basePath . '/assets/content/media/' . $model->content);
                    }
                ...
                $files = json_decode($_POST['Content']['files']);
                foreach ($files as $file) {
                    $model = new Content;
                    $model->setAttributes($_POST['Content']);
                    $model->user_id = Yii::app()->user->id;

                    $model->content = $file->file_name;
     ...
     */
    public function actionPostfile()
    {
        $model = new Content;
        $out = new stdClass();
        $model->setScenario('media');
        $model->media = CUploadedFile::getInstance($model, 'media');
        $model->validate('media');
        $t = time();
        $out->file_name = Yii::app()->user->id . "/{$t}-{$model->media->name}";
        $contentFileDir = Yii::app()->basePath . '/assets/content/media/';
        $usrFileDir = $contentFileDir . Yii::app()->user->id . '/';

        if(!is_dir($usrFileDir))
            mkdir($usrFileDir);

        $model->media->saveAs($contentFileDir . $out->file_name);
    ...
    {
        $model = Content::model();
        $feeds = new CActiveDataProvider('Feed', array(
            'criteria' => array(
    ...
    public function actionDelete($id, $action = true)
    {
        ...
        if ($model->content_type == 'media')
            unlink(Yii::app()->basePath . '/assets/content/media/' . $model->content);
